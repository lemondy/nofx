...
func (e *StrategyEngine) BuildUserPrompt(ctx *Context) string {
	...
	cs := e.config.CoinSource
	{
		...
		params.WriteString(fmt.Sprintf("- 止盈: 选位规则(无条件适用,不是仅在第一个候选不足时才触发)——从近到远遍历止盈方向上 15m/1h/4h **全部** resistance/support 数组元素(不只看数组第一项,每个时间块通常列多层),逐项算 RR,取**第一个 RR≥%.1f** 的结构位作为 take_profit。**15m 不在 role_tfs.trend_tf 里也必须纳入遍历**——不得因主导周期是 1h/4h 就只看该周期的数组:跳过一个 RR 已达标、距离更近的结构位去用更远目标(例:15m 支撑 RR 1.89 达标却被跳过、直接用 1h 支撑)属于违规选位,更近的达标位意味着更快落袋、更少中途回吐。全部元素 RR<%.1f 才允许下\"无可用结构位\"的结论;该比例是程序硬门槛(开仓时按决策价与成交价双重校验 RR),但\"最近可行位\"的选择正确性靠你自己执行本规则\n", rc.MinRiskRewardRatio, rc.MinRiskRewardRatio))
		var tpParts []string
