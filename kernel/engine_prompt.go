...
func (e *StrategyEngine) BuildSystemPrompt(accountEquity float64, variant string) string {
	...
	if riskControl.LimitEntryEnabled {
		...
		sb.WriteString("- `open_long` / `open_short`(市价,两类例外,其余情况必须用限价): **例外一(突破追入)**: 当且仅当以下条件**全部**满足——① `breakout.status`=\"confirmed\"(仅此值;approach/broken_unconfirmed/retest_hold 均不算)② `breakout.volume_confirmation`=true ③ `breakout.oi_confirmation`=true ④ `directional_score`≥80 ⑤ `signal_conflict.directional_conflict`=false ⑥ 该币未处于连亏禁开仓期。**例外二(布林上轨骑行)**: 程序预计算标志 `bb_ride.ride`=true(短期量能暴增 + 15m 连续 ≥3 根阳线收盘且高点贴上轨,见快照 `bb_ride.windows`/`volume_surge`/`upper_band`)且你的 confidence≥80。两类例外任一成立才允许 `open_long`/`open_short` 市价追入,reasoning 逐条列出成立条件;任一例外不满足仍必须用限价单等回踩,市价承担滑点与假突破成本。突破触发单(stop-entry)系统当前不支持,不要输出其他动作类型。\n")
	}
...
func (e *StrategyEngine) BuildUserPrompt(ctx *Context) string {
	...
	cs := e.config.CoinSource
	{
		...
		if cs.SourceType == "short_scan" || (cs.SourceType == "mixed" && cs.UseShortScan) {
			...
			annPct := frPct * 1095
			params.WriteString(fmt.Sprintf("- 做空资金费率拥挤(二选一,均为支撑证据而非必要条件): ① 费率年化 > %.1f%%(8h 结算口径即每期费率 > %.4f%%;非 8h 结算的币按真实结算间隔换算)——多头拥挤进行时; 或 ② funding_rollover 费率刚从高位回落——拥挤开始瓦解,通常是更好的入场时机(两者都不满足时不要仅因费率理由做空)。磨顶宇宙(universe=\"near_high\")的候选豁免此条件——缓慢磨顶的币费率通常已正常化,其确认信号是 4h 顶背离。费率漂移快,一律以 Structured Signal 实时值为准\n", annPct, frPct))
		}
		...
		stopBand := "上限 = max(2×ATR(4h), 8%)——缓冲与下限看 1h 节奏,宽止损上限看 4h 波动,二者取宽不取窄;ATR 用各币 JSON 里对应时间块的 atr_pct;结构位落在区间外时放弃该设置,不要硬凑"
		...
		params.WriteString(fmt.Sprintf("- 止盈: 优先下一个结构位(resistance/support 数组已给出距离);不足 min_risk_reward_ratio(%.1f) 时换更远结构位。\"更远\"必须逐项检查止盈方向上 15m/1h/4h 全部 resistance/support 数组元素(不只看数组第一项——每个时间块通常列多层),从近到远逐个算 RR,取第一个能过 %.1f 门槛的;全部不达标才允许下\"无可用结构位\"的结论,该比例是硬门槛\n", rc.MinRiskRewardRatio, rc.MinRiskRewardRatio))
		var tpParts []string
	...
	sb.WriteString("> **short_scan 候选的默认姿态(稳定规则,勿逐次重判)**: short_scan 按涨幅大入选,候选的 1h/4h 结构天然还是多头——scanner 说可空、结构说多头不是偶发冲突,是该引擎的常态。默认姿态: 顶部确认信号(顶背离/假突破/破 EMA20/资金费回落)之外,**还必须 execution_filter.short_allowed=true(15m 微趋势已转)才允许做空**;仅凭确认信号而 15m 仍 up → 输出 wait + wait_bias=short,等微趋势转向。entry_timing_gate 开启时这同时是硬规则(15m 逆势 open_short 会被程序拒单)\n\n")
	...
	if ctx.OIRankingData != nil {
		...
		sb.WriteString("\n> ⚠️ 边界: 以上排行仅为市场整体情绪/资金轮动的宏观参考(聚合口径,条目未必可交易)。**实际可交易标的仅限上方 Candidate Coins 里带完整结构化信号的 symbol** —— 不要对榜单中出现但不在候选池里的币做任何决策,也不要把榜单数字当作这些币的完整行情。\n")
	}

	// NetFlow Ranking data (market-wide fund flow)
	if ctx.NetFlowRankingData != nil {
		sb.WriteString(nofxos.FormatNetFlowRankingForAI(ctx.NetFlowRankingData, nofxosLang, interestingSymbols(ctx)))
		sb.WriteString("\n> ⚠️ 边界: 以上资金流榜单仅为宏观情绪参考。实际可交易标的仅限 Candidate Coins 中带完整结构化信号的 symbol。\n")
	}
	...
	if ctx.PriceRankingData != nil {
		...
		sb.WriteString("\n> ⚠️ 边界: 以上涨跌幅榜单仅为市场情绪/资金轮动的宏观参考。实际可交易标的仅限 Candidate Coins 中带完整结构化信号的 symbol。\n")
	}
