...
// WithStreamDecisions enables/disables SSE streaming for AI calls.
// When enabled (and the provider is OpenAI-compatible), decision calls stream:
// headers return immediately and the 60s idle watchdog bounds a hung
// connection, instead of the full-request timeout killing long generations.
func WithStreamDecisions(enabled bool) ClientOption {
	return func(c *Config) {
		c.StreamDecisions = enabled
	}
}
...
// WithGLMConfig sets GLM (Zhipu BigModel) configuration
//
// Usage example:
//
//	client := mcp.NewClient(mcp.WithGLMConfig("sk-xxx"))
func WithGLMConfig(apiKey string) ClientOption {
	return func(c *Config) {
		c.Provider = ProviderGLM
		c.APIKey = apiKey
		c.BaseURL = DefaultGLMBaseURL
