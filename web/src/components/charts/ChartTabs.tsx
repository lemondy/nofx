...
export function ChartTabs({ traderId, selectedSymbol, updateKey, exchangeId }: ChartTabsProps) {
 ...
 return (
 <div className={`nofx-glass rounded-lg border border-nofx-line/50 relative z-10 w-full flex flex-col transition-all duration-300 ${typeof window !== 'undefined' && window.innerWidth < 768 ? 'h-[500px]' : 'h-[600px]'
 }`}>
 {/* 
 ...
 */}
 ...
 {activeTab === 'kline' && (
 <div className="hidden md:flex items-center gap-1 ml-2 border-l border-nofx-line pl-2">
 ...
 return (
 <div key={category}>
 <div className="px-3 py-1.5 text-[9px] font-bold text-nofx-text-muted/60 bg-[#1E1E1A]/5 uppercase tracking-wider">{labels[category]}</div>
 ...
 <span className="px-2.5 py-1 bg-[#E9E4D6] border border-nofx-line rounded text-[11px] font-bold text-nofx-text-main font-mono">{chartSymbol}</span>
 )}
 </div>
 ...
 />
 <button type="submit" className="px-2 py-1 bg-[#1E1E1A]/5 border border-nofx-line border-l-0 rounded-r text-[10px] text-nofx-text-muted hover:text-[#1E1E1A] hover:bg-[#1E1E1A]/10 transition-all">
 Go
 </button>
 </form>
 </div>
