...
export default function TerminalHero() {
 ...
 className="mb-8 inline-flex items-center gap-3 px-4 py-2 rounded-full border border-nofx-gold/20 bg-nofx-gold/5 "
 >
 ...
 AGENTIC<br />
 <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#B8912A] via-[#E3D6A3] to-[#B8912A] animate-shimmer bg-[length:200%_auto] tracking-tight">TRADING</span>
 </h1>
 ...
 <span className="text-lg md:text-xl font-bold text-[#1E1E1A] tracking-wider group-hover:text-nofx-gold transition-colors">{market}</span>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="w-full max-w-lg h-12 bg-[#F2EFE6] border border-[#C0B9A2] rounded flex items-center px-4 mb-10 font-mono text-sm shadow-2xl group hover:border-nofx-gold/50 transition-colors cursor-text" onClick={() => document.getElementById('market-scanner')?.scrollIntoView({ behavior: 'smooth' })}>
 ...
 <div className="absolute top-0 right-0 h-full w-[50vw] hidden 2xl:flex flex-col items-end justify-end pr-8 pb-20 z-10">
 {/* Subtle gradient orb */}
 ...
 <span className="flex items-center gap-2"><Globe className="w-3 h-3 text-[#8A8A7C]" /> GLOBAL MARKET ACCESS</span>
...
function CommunityStats() {
 ...
 <span className="text-[10px] font-mono text-[#6E6E60] tracking-wider">{stat.label}</span>
 </div>
 <span className="text-xl font-bold font-mono text-[#1E1E1A] group-hover:text-nofx-gold transition-colors">{stat.value}</span>
 </a>
 ))}
 </div>
 )
