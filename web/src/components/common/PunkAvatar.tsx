...
const BACKGROUNDS = [
 '#1a1a2e', '#E4E0D0', '#0f3460', '#1b1b2f', '#E4E0D0',
...
const ACCESSORY_COLORS = [
 '#B8912A', '#2E7D4F', '#C0392B', '#5e7a5e', '#a855f7',
...
export function PunkAvatar({ seed, size = 40, className = '' }: PunkAvatarProps) {
 ...
 return (
 <>
 <rect x={7*px} y={4*px} width={10*px} height={1*px} fill="#B8912A" />
 <rect x={8*px} y={2*px} width={2*px} height={2*px} fill="#B8912A" />
 <rect x={11*px} y={1*px} width={2*px} height={3*px} fill="#B8912A" />
 <rect x={14*px} y={2*px} width={2*px} height={2*px} fill="#B8912A" />
 </>
 )
 default:
 return null
 ...
 if (hasEarring) {
 elements.push(
 <circle key="earring" cx={5*px} cy={12*px} r={px} fill="#B8912A" />
 )
 }
 ...
 elements.push(
 <g key="laser">
 <rect x={9*px} y={10*px} width={15*px} height={2*px} fill="#C0392B" opacity={0.8} />
